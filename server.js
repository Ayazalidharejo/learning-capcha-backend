...
//     const user = await UserModel.findOne({ email: username.toLowerCase() }).exec();
...
const allowedOrigins = [
  'https://learning-capcha.vercel.app',
  'http://localhost:5173',
  'http://localhost:3030'
];
...
app.use(cors({
  origin: '*',  // Or use allowedOrigins array
  credentials: true,
...
// ... (rest of your API routes remain same) ...
...
// ✅ CRITICAL: Proper Vercel export
if (process.env.VERCEL) {
  // Initialize MongoDB on cold start
  connectMongo().catch(e => console.error('MongoDB init failed:', e));
